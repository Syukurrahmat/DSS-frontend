...
import { toFormatedDate, toFormatedDatetime } from '@/lib/dateFormating.utils';
...
export default function DetailNode() {
	...
	const { data, mutate, error } = useSWR<NodeDataPage>(apiURLEntryPoint);
	...
	const statusAtt = nodeStatusAttr[data.isUptodate ? 'active' : 'inactive'];
	const ownshipAtt = nodeTypeAttr[data.companyId ? 'private' : 'public'];

	const canEdit =
		roleIs('admin') ||
		(roleIs('manager') && data.owner?.managedBy == user.id);
