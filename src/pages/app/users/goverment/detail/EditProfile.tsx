...
import { compareObjects, trimAndCleanProps } from '@/lib/common.utils';
import { myAxios } from '@/lib/fetcher.utils';
import * as valSchema from '@/lib/validator.utils';
import { DetailAdmin, DetailGoverment } from '@/types/data/users';
...
interface IEUModal extends ButtonProps {
	data: DetailGoverment;
	mutate: KeyedMutator<DetailGoverment>;
}

export default function EditAdminProfileButton({ data, mutate, ...rest }: IEUModal) {
	const { name } = data;
	...
	const userId = useMatch('/account') ? user.id : id;
